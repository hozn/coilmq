...
def make_sa():
    """
    Creates a SQLAlchemy queue store, pulling config values from the CoilMQ configuration.
    """
    configuration = dict(config.items('coilmq'))
    meta.engine = engine_from_config(configuration, 'qstore.sa.')
...
class SAQueue(QueueStore):
    ...
    def dequeue(self, destination):
        ...
        try:
            ...
            selstmt = select([model.frames_table.c.message_id, model.frames_table.c.frame])
            selstmt = selstmt.where(model.frames_table.c.destination==destination)
            selstmt = selstmt.order_by(model.frames_table.c.queued, model.frames_table.c.sequence)
            ...
            delstmt = model.frames_table.delete().where(model.frames_table.c.message_id==first[model.frames_table.c.message_id])
    ...
    def has_frames(self, destination):
        ...
        """
        session = meta.Session()
        sel = select([model.frames_table.c.message_id]).where(model.frames_table.c.destination==destination)
    ...
    def size(self, destination):
        ...
        """
        session = meta.Session()
        sel = select([func.count(model.frames_table.c.message_id)]).where(model.frames_table.c.destination==destination)
        result = session.execute(sel)
        first = result.fetchone()
        if not first:
            return 0
