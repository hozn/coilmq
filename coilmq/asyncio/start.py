# !python
"""
Entrypoint for starting the asyncio application.
"""
import os
import logging
import asyncio
...
from coilmq.asyncio.server.socket_server import StompHandler
...
async def server_from_config(config=None, server_class=None, additional_kwargs=None):
    """
    ...
    server = await asyncio.start_server(stomp_handler.on_connected, config.get('coilmq', 'listen_addr'), config.getint('coilmq', 'listen_port'))
...
async def run_server(configfile, logfile, debug):
    global global_config
    ...
    try:
        ...
        server = await server_from_config()
        logger.info("Stomp server listening on %s:%s" % server.sockets[0].getsockname())
...
def _main(config=None, host=None, port=None, logfile=None, debug=None,
    ...
    asyncio.run(run_server(config, logfile, debug))
...
@click.command()
@click.option("-c", "--config", help="Read configuration from FILE. (CLI options override config file.)",
              metavar="FILE")
...
@click.option("-l", "--logfile", help="Log to specified file (unless logging configured in config file).",
              metavar="FILE")
